...
class Broker
{
    ...
    {
        $this->log = new Logger("BROKER.$broker");
        $handler = new StreamHandler('php://stdout', Logger::DEBUG);
        $this->log->pushHandler($handler);
        
        $this->log->debug("construct", array("url" => $url, "broker" =>$broker, "secret" =>$secret, "cookie_lifetime" =>$cookie_lifetime ));
        ...
        $this->log->debug("construct", array("token" => $this->token ));
    }
    ...
    public function getAttachUrl($params = [])
    {
        ...
        $this->log->debug("getAttachUrl", array("url" => $this->url, "data" => $data, "params" => $params));
    ...
    {
        $this->log->debug("attach", array("init"));

        if ($this->isAttached()) return;

        $this->log->debug("attach", array("not attached"));
        ...
        $this->log->debug("return_url", array($url));

        header("Location: $url", true, 307);
        // header( "// Refresh:5; url=$url", true, 303);
        $this->log->debug("attach", array("redirecting"));
    ...
    protected function request($method, $command, $data = null)
    {
        ...
        $logData = array("method" => $method, "data" =>$data, "command" =>$command, "url" => $url);
        ...
        $logData["data"] = $data;
        $this->log->debug("request", $logData);
